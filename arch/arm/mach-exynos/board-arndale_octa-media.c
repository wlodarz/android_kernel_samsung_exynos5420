...
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_LITE_MODULE
#if defined(CONFIG_VIDEO_S5K6B2) && defined(CONFIG_VISION_MODE)
static struct exynos5_sensor_gpio_info gpio_vision_sensor_arndale_octa = {
	.cfg = {
		/* 2M AVDD_28V */
		/* 2M DVDD_18V */
		/* 2M VIS_STBY */
...
static struct exynos_fimc_is_sensor_platform_data s5k6b2_platdata = {
	...
	.gpio_info = &gpio_vision_sensor_arndale_octa,
...
#endif
#if defined (CONFIG_VIDEO_S5K5EA_MODULE)

#define GPIO_CAM0_SDA EXYNOS5420_GPB3(6)
#define GPIO_CAM0_SCL EXYNOS5420_GPB3(7)
#define GPIO_CAM0_MCLK EXYNOS5420_GPH0(5)	// CIS_CLK0
#define GPIO_CAM1_MCLK EXYNOS5420_GPH0(6)	// CIS_CLK1

struct i2c_board_info i2c_devs_cam2m14[] = {
	{
		I2C_BOARD_INFO("S5K5EA", 0x2D),
	}
};
EXPORT_SYMBOL(i2c_devs_cam2m14);

//platform_device
static struct i2c_gpio_platform_data gpio_i2c14_data = {
	.sda_pin            = GPIO_CAM0_SDA,
	.scl_pin            = GPIO_CAM0_SCL,
	.udelay             = 5, 
	.sda_is_open_drain  = 0, 
	.scl_is_open_drain  = 0, 
...
static struct platform_device gpio_i2c14_dev = {
	.name               = "i2c-gpio",
	.id                 = 14,
	.dev.platform_data  = &gpio_i2c14_data,
};

static void __init arndale_octa_camera_gpio_cfg(void)
{
	// gpio - i2c pin 
	gpio_request(GPIO_CAM0_SDA, "GPIO_MAIN_CAM_SDA_18V");
	s3c_gpio_cfgpin(GPIO_CAM0_SDA, S3C_GPIO_SFN(0x2));
	s3c_gpio_setpull(GPIO_CAM0_SDA, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_CAM0_SDA);
	
	gpio_request(GPIO_CAM0_SCL, "GPIO_MAIN_CAM_SCL_18V");
	s3c_gpio_cfgpin(GPIO_CAM0_SCL, S3C_GPIO_SFN(0x2 << 4));
	s3c_gpio_setpull(GPIO_CAM0_SCL, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_CAM0_SCL);

	s5p_gpio_set_drvstr(GPIO_CAM0_SDA, S5P_GPIO_DRVSTR_LV2);
	s5p_gpio_set_drvstr(GPIO_CAM0_SCL, S5P_GPIO_DRVSTR_LV2);
	s3c_gpio_setpull(GPIO_CAM0_SDA, S3C_GPIO_PULL_UP);
	s3c_gpio_setpull(GPIO_CAM0_SCL, S3C_GPIO_PULL_UP);

	// back cam mclk out set
	gpio_request(GPIO_CAM0_MCLK, "BACK_MCLK");
	s3c_gpio_cfgpin(GPIO_CAM0_MCLK, S3C_GPIO_SFN(0x2));
	s3c_gpio_setpull(GPIO_CAM0_MCLK, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_CAM0_MCLK);
}
#endif
#endif	/* CONFIG_VIDEO_EXYNOS_FIMC_LITE_MODULE */

#ifdef CONFIG_VIDEO_EXYNOS_MIPI_CSIS_MODULE
...
static struct s5p_mfc_qos arndale_octa_mfc_qos_table[] = {
	[0] = {
		.thrd_mb	= 0,
		.freq_mfc	= 111000,
		.freq_int	= 111000,
...
static struct s5p_mfc_platdata arndale_octa_mfc_pd = {
	.ip_ver		= IP_VER_MFC_6A_0,
	.clock_rate	= 333 * MHZ,
	.min_rate	= 84000, /* in KHz */
#ifdef CONFIG_ARM_EXYNOS5420_BUS_DEVFREQ
	.num_qos_steps	= ARRAY_SIZE(arndale_octa_mfc_qos_table),
	.qos_table	= arndale_octa_mfc_qos_table,
#endif
};
...
static struct platform_device *arndale_octa_media_devices[] __initdata = {
...
#ifdef CONFIG_VIDEO_EXYNOS5_FIMC_IS
static struct exynos5_sensor_gpio_info gpio_arndale_octa = {
	.cfg = {
		/* 13M AVDD_28V */
		{
			.pin_type = PIN_GPIO,
...
#endif
#if defined (CONFIG_VIDEO_S5K5EA_MODULE)
struct exynos_isp_info s5k5ea = {
	.board_info	= i2c_devs_cam2m14, 
	.cam_srclk_name	= "sclk_isp_sensor0",
	.clk_frequency	= 24000000UL,
	.bus_type	= CAM_TYPE_MIPI,
...
EXPORT_SYMBOL(s5k5ea);

/* This is for platdata of fimc-lite */
struct s3c_platform_camera flite_s5k5ea = {
	.type		= CAM_TYPE_MIPI,
	.use_isp	= false,
	.inv_pclk	= 1,
	.inv_vsync	= 1,
...
EXPORT_SYMBOL(flite_s5k5ea);
#endif

#if defined(CONFIG_VIDEO_EXYNOS_FIMC_LITE_MODULE)
...
static void __init arndale_octa_set_camera_platdata(void)
{
#if defined(CONFIG_VIDEO_S5K6B2) && defined(CONFIG_VISION_MODE)
	int gsc_cam_index = 0;
	...
	exynos_gsc1_default_data.isp_info[gsc_cam_index++] = &s5k5ea;

	exynos_flite1_default_data.cam[flite1_cam_index] = &flite_s5k5ea;
	exynos_flite1_default_data.isp_info[flite1_cam_index] = &s5k5ea;
	flite1_cam_index++;
...
void __init exynos5_arndale_octa_media_init(void)
{
#if defined (CONFIG_CSI_D) || defined (CONFIG_S5K6B2_CSI_D)
	...
	s5p_mfc_set_platdata(&arndale_octa_mfc_pd);
	...
	platform_add_devices(arndale_octa_media_devices,
			ARRAY_SIZE(arndale_octa_media_devices));
...
#endif
#if defined (CONFIG_VIDEO_S5K5EA_MODULE)
	__set_mipi_csi_config(&s5p_mipi_csis0_default_data, s5k5ea.csi_data_align);
#endif
...
#ifdef CONFIG_VIDEO_EXYNOS_FIMC_LITE_MODULE
#if defined (CONFIG_VIDEO_S5K5EA_MODULE)
	arndale_octa_camera_gpio_cfg();
#endif
	arndale_octa_set_camera_platdata();
	...
	exynos5_fimc_is_data.gpio_info = &gpio_arndale_octa;
