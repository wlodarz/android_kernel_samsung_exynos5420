...
#define ARNDALE_OCTA_PMIC_EINT	IRQ_EINT(26)
...
{
	unsigned int pin = irq_to_gpio(ARNDALE_OCTA_PMIC_EINT);
...
static struct exynos_devfreq_platdata arndale_octa_qos_mif_pd __initdata = {
	.default_qos = 133000,
};
...
static struct exynos_devfreq_platdata arndale_octa_qos_int_pd __initdata = {
	.default_qos = 83000,
};
#endif

static struct platform_device *arndale_octa_power_devices[] __initdata = {
...
void __init exynos5_arndale_octa_power_init(void)
{
...
#ifdef CONFIG_ARM_EXYNOS5420_BUS_DEVFREQ
	s3c_set_platdata(&arndale_octa_qos_mif_pd, sizeof(struct exynos_devfreq_platdata),
			&exynos5_mif_devfreq);
	s3c_set_platdata(&arndale_octa_qos_int_pd, sizeof(struct exynos_devfreq_platdata),
			&exynos5_int_devfreq);
#endif

	platform_add_devices(arndale_octa_power_devices,
			ARRAY_SIZE(arndale_octa_power_devices));
}
